...
class Cache implements \ArrayAccess {
     ...
     *
     * @param string $backend The name of the backend to use.  Currently 'apc', 'file', 'memcached', 'session' and
     * 'sqlite' are supported.
     *
     * @param array $options Options to pass on to the backend.  These will also be specific to the backend
     * you are using.
     *
    ...
    function __construct($backend = NULL, $options = array(), $namespace = 'default') {
        ...
        if(! $backend)
            $backend = in_array('apc', get_loaded_extensions()) ? 'apc' : 'session';

        if(is_array($backend)) {

            /**
             * If we get an array then we're using the chain backend.
             */
            $this->backend = new Cache\Backend\Chain($backend, $options);

        } else {

            /*
             * Load the backend and with backend options and send it to the frontend
             */
     ...
     *
     * @param mixed $key The reference key used to store the value.
     *
     * @param bool $default If the value doesn't exist, this default will be returned instead.
     *
     * @param bool $save_default If the value doesn't exist and a default is specified, save that default to cache.
     *
     ...
     *
     * @param mixed $key The reference key under which to store the value.
     *
     ...
     *
     * @param mixed $timeout The number of seconds after which the value becomes invalid.  If not set the global
     ...
     *
     * @param mixed $key The value key to check for.
     *
