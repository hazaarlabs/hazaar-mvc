...
class Session extends \Hazaar\Cache\Backend {
    ...
    /**
     * @detail      The session backend init method is responsible for maintaining a valid session.  If a session
     ...
     * @since       1.0.0
     *
     * @param       string $namespace The namespace to use for this session
     */
    function init($namespace) {
        ...
        if(! (array_key_exists($namespace, $_SESSION[APPLICATION_BASE]) && is_array($_SESSION[APPLICATION_BASE][$namespace])))
            $_SESSION[APPLICATION_BASE][$namespace] = array();

        $this->values =& $_SESSION[APPLICATION_BASE][$namespace];
     ...
     *
     * @param      mixed $key The key name of the data field to return.
     * @param      mixed $default An optional default value to set and return if the field currently has no value.
     *
     * @return mixed
     */
    ...
    /**
     * @detail      Clears all values from all applications.  This equates to a full session reset.
     *
