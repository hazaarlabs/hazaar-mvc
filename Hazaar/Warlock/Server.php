...
class Client {
    ...
    public $subscriptions = array();  //This is an array of event_id and socket pairs
...
class Server extends WebSockets {
    ...
    private        $running  = NULL;      // Main loop state. On false, Warlock will exit the main loop and terminate

    private        $shutdown = NULL;

    public         $config;

    private        $tags     = array();   // Job tags

    private        $start    = 0;         // Epoch of when Warlock was started

    private        $pid      = 0;         // Current process id

    private        $pidfile;             //Current process id file
     ...
     * JOBS & SERVICES
     */
    private $procs    = array();        //Currently running processes (jobs AND services)

    private $jobQueue = array();        //Main job queue

    private $services = array();        //Application services

    /**
     * QUEUES
     */
    private $waitQueue  = array();      //The wait queue. Clients subscribe to events and are added to this array.

    private $eventQueue = array();      //The Event queue.  Holds active events waiting to be seen.

    /**
     * SOCKETS & STREAMS
     */
    private $master        = NULL;      //The main socket for listening for incomming connections.

    private $sockets       = array();   //Currently connected sockets we are listening for data on.

    private $client_lookup = array();   //Socket to client lookup table.  Required as clients can have multiple connections.

    private $clients       = array();   //Currently connected clients.

    private $protocol;                  //The Warlock protocol encoder/decoder.
    ...
    public function run() {
        ...
        if($services->loaded()) {
            ...
            foreach($services as $name => $options) {
                ...
                $this->services[$name] = $options->extend(array('name' => $name))
                                                 ->toArray();
    ...
    private function processClient($socket) {
        ...
        if(! $client instanceof Client) {
            ...
            /**
             * Sometimes we can get multiple frames in a single buffer so we cycle through them until they are all processed.  This will
            ...
            while($frame = $this->processFrame($buf, $client)) {
                ...
                if($type = $this->protocol->decode($frame, $payload, $offset)) {
                    ...
                    $this->send($socket, $this->protocol->encode('error', array('reason' => $reason)));
    ...
    private function initiateHandshake($socket, $request) {
        ...
        if(array_key_exists('connection', $headers) && preg_match('/upgrade/', strtolower($headers['connection']))) {
            ...
            if($type) {
                ...
                $response = $this->httpResponse(200, $this->protocol->encode('error', array('reason' => $reason)), $responseHeaders);
            ...
            if($count > 0) {
                ...
                if(isset($array[1])) {
                    ...
                    if($name == 'events' && array_key_exists($this->config->admin->trigger, $array[0])) {
                        ...
                        $status[$name] = array_diff_key($array[0], array_flip(array($this->config->admin->trigger)));
    ...
    private function cancelJob($job_id) {
        ...
        if(! array_key_exists($job_id, $this->jobQueue) || $this->jobQueue[$job_id]['status'] != STATUS_QUEUED ||
           $this->jobQueue[$job_id]['status'] != STATUS_QUEUED_RETRY
        ) {
    ...
    private function processJobs() {
        ...
        foreach($this->jobQueue as $id => &$job) {
            ...
            if(($job['status'] == STATUS_QUEUED || $job['status'] == STATUS_QUEUED_RETRY) && $now >= $job['start']) {
                ...
                $env = array(
                    ...
                    'WARLOCK_SID'      => $this->config->sys->id
                );
        ...
        foreach($this->procs as $id => &$proc) {
            ...
            if($status['running'] === FALSE) {
                ...
                $read = array($proc['pipes'][1], $proc['pipes'][2]);
     ...
     *              queue that the client has not yet seen.  If there are, the first event found is sent to the client,
    ...
    /**
     * @detail      This method is executed when a event is triggered. It is responsible for sending events to clients
    ...
    private function ping($client_id) {
        ...
        if($client_id == 'all') {
            ...
            $clients[] = array($this->clients[$client_id]);
