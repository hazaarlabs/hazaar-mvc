...
class Date extends \Datetime {
    ...
    /**
     * @detail      The Date class constructor takes two values.  A datetime declaration and a timezone.  Both are
     ...
     *
     * @param       string $timezone The timezone for this datetime value.  If no timezone is specified then the default
     *                               timezone is used.
     */
    public function __construct($datetime = NULL, $timezone = NULL) {
        ...
        } elseif(is_array($datetime) && array_key_exists('sec', $datetime)) {  //Array version of MongoDate
     ...
     *
     * @param Mixed $timezone The timezone to set.  This can be either a [[DateTimeZone]] object, a string
     ...
     *
     * @param string $format A standard date format string
     *
     * @param string $timezone The timezone to use to convert the date time value.
     *
    ...
    /**
     * Magic method to output the datetime value as a string.  Uses the Date::timestamp() method.
     *
     ...
     *
     * @param   integer $precision The number of digits to round an age to.  Default: 0
     *
     ...
     *
     * @param \DateTime $timestamp The timestamp to compare the current date/time to.
     *
     * @param bool $return_seconds If TRUE then the return value will be an integer indicating the difference in whole seconds.
     *
    ...
    public function diff($timestamp, $return_seconds = FALSE) {
        ...
        if($return_seconds) {
            ...
            if($diff->days > 0) $seconds += $diff->days * 86400;

            if($diff->h > 0) $seconds += $diff->h * 3600;

            if($diff->i > 0) $seconds += $diff->i * 60;

            if($diff->s > 0) $seconds += $diff->s;

            if($diff->invert == 0) $seconds = -($seconds);
