...
abstract class WebSocketServer extends Server {
     ...
     * created, but before the handshake has completed.
     */
                ...
                if ($socket == $this->master) {
                    ...
                    if ($numBytes === false) {
                        ...
                        if (! $user->handshake) {
                            ...
                            if (strpos($tmp, "\n\n") === false) {
                                continue; // If the client has not finished
                                              // sending the header, then wait
                                              // before sending our upgrade
                                              // response.
                            }
                            ...
                            if (($message = $this->deframe($buffer, $user)) !== FALSE) {
                                if ($user->hasSentClose) {
                                    ...
                                    $this->process($user, $message); // todo:
                                                                         // Re-check
                                                                         // this.
                                                                         // Should
                                                                         // already
    ...
    protected function doHandshake($user, $buffer) {
        ...
        if (($this->headerSecWebSocketProtocolRequired && ! isset($headers['sec-websocket-protocol'])) || ($this->headerSecWebSocketProtocolRequired &&
                 ! $this->checkWebsocProtocol($headers['sec-websocket-protocol']))) {
            $handshakeResponse = "HTTP/1.1 400 Bad Request";
        }
        if (($this->headerSecWebSocketExtensionsRequired && ! isset($headers['sec-websocket-extensions'])) || ($this->headerSecWebSocketExtensionsRequired &&
                 ! $this->checkWebsocExtensions($headers['sec-websocket-extensions']))) {
            $handshakeResponse = "HTTP/1.1 400 Bad Request";
        }
        
        // Done verifying the _required_ headers and optionally required
        // headers.
        ...
        $subProtocol = (isset($headers['sec-websocket-protocol'])) ? $this->processProtocol(
                $headers['sec-websocket-protocol']) : "";
        $extensions = (isset($headers['sec-websocket-extensions'])) ? $this->processExtensions(
    ...
    protected function processProtocol($protocol) {
        ...
        return ""; // return either "Sec-WebSocket-Protocol:
                   ...
                   // The carriage return/newline combo must appear at the end of a
                   // non-empty string, and must not
                   // appear at the beginning of the string nor in an otherwise
                   // empty string, or it will be considered part of
                   // the response body, which will trigger an error in the client
                   // as it will not be formatted correctly.
    }
    ...
    protected function extractHeaders($message) {
        ...
        if ($header['length'] == 126) {
            ...
            $header['length'] = ord($message[2]) * 65536 * 65536 * 65536 * 256 + ord($message[3]) * 65536 * 65536 * 65536 +
                     ord($message[4]) * 65536 * 65536 * 256 + ord($message[5]) * 65536 * 65536 +
                     ord($message[6]) * 65536 * 256 + ord($message[7]) * 65536 + ord($message[8]) * 256 +
                     ord($message[9]);
