...
class Warlock extends \Hazaar\Controller {
    ...
    private function start() {
        ...
        $out = new \Hazaar\Controller\Response\Json(array('result' => 'error'));
    ...
    private function controlPanel($params) {
        ...
        $out->addHelper('widget', array('theme' => 'ui-lightness'));
        ...
        $out->service = array('running' => $this->control->isRunning());
        ...
        foreach($dataSources as $dsname) {
            ...
            foreach($graph['ticks'] as $tick => $count) {
                ...
                $data[] = array('tick' => date('H:i:s', $tick), 'value' => $count);
    ...
    private function test($serviceName) {
        ...
        $out = new Response\Json(array('result' => 'err'));
