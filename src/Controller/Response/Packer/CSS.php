...
namespace Hazaar\Controller\Response\Packer;
...
class CSSmin {
    ...
    private function normalize_int($size) {
        if (is_string($size)) {
            $i_size = intval(substr($size, 0, strlen($size)-1));
            switch (substr($size, -1)) {
                case 'M':
                case 'm': return $i_size * 1048576;
                case 'K':
                case 'k': return $i_size * 1024;
                case 'G':
                case 'g': return $i_size * 1073741824;
            }
        }
